# NEXUS-PRO Manifest

Version: `7.0-pro-enterprise`

Skill enterprise para Antigravity orientado al desarrollo full stack profesional con Laravel, React y PostgreSQL. Aplicable a SaaS multi-tenant, POS, e-commerce, paneles administrativos y cualquier proyecto que requiera arquitectura limpia, seguridad robusta y preparación para producción.

## Archivos incluidos

- `CHANGELOG.md`
- `MANIFEST.md`
- `SKILL.md`
- `examples/architecture-review-prompt.md`
- `examples/bugfix-prompt.md`
- `examples/create-endpoint-prompt.md`
- `examples/dependency-review-prompt.md`
- `examples/export-feature-prompt.md`
- `examples/file-upload-feature-prompt.md`
- `examples/idempotent-endpoint-prompt.md`
- `examples/migration-safe-prompt.md`
- `examples/new-feature-prompt.md`
- `examples/production-release-prompt.md`
- `examples/refactor-prompt.md`
- `examples/security-review-prompt.md`
- `examples/tenant-isolation-review-prompt.md`
- `references/accessibility.md`
- `references/ai-coding-constraints.md`
- `references/anti-hallucination.md`
- `references/api-standards.md`
- `references/architecture-review-mode.md`
- `references/architecture.md`
- `references/audit-trail.md`
- `references/backend.md`
- `references/business-rules-registry.md`
- `references/cicd.md`
- `references/contract-testing.md`
- `references/data-classification.md`
- `references/data-privacy.md`
- `references/database.md`
- `references/ddd-lite.md`
- `references/decision-trees.md`
- `references/definition-of-done.md`
- `references/deletion-policy.md`
- `references/dependency-governance.md`
- `references/design-system.md`
- `references/disaster-recovery.md`
- `references/documentation.md`
- `references/environment-parity.md`
- `references/error-taxonomy.md`
- `references/event-driven.md`
- `references/execution-policies.md`
- `references/export-safety.md`
- `references/feature-flags.md`
- `references/file-upload-security.md`
- `references/frontend.md`
- `references/git-strategy.md`
- `references/idempotency.md`
- `references/infrastructure.md`
- `references/internationalization.md`
- `references/migration-safety.md`
- `references/mobile-pwa.md`
- `references/monitoring.md`
- `references/observability-engineering.md`
- `references/openapi.md`
- `references/performance.md`
- `references/plan-before-code.md`
- `references/production-readiness-gates.md`
- `references/prompt-contracts.md`
- `references/queue-reliability.md`
- `references/scalability.md`
- `references/scope-control.md`
- `references/security.md`
- `references/tenant-isolation.md`
- `references/testing.md`
- `references/ux-failure-states.md`
- `templates/api-endpoint-template.md`
- `templates/architecture-review-report.md`
- `templates/audit-event-template.md`
- `templates/backend-service-template.md`
- `templates/code-review-checklist.md`
- `templates/ddd-use-case-template.md`
- `templates/export-job-template.md`
- `templates/feature-flag-template.md`
- `templates/file-upload-service-template.md`
- `templates/frontend-module-template.md`
- `templates/idempotency-middleware-template.md`
- `templates/migration-template.md`
- `templates/module-readme-template.md`
- `templates/openapi-endpoint-template.md`
- `templates/production-readiness-checklist.md`

## Notas

La ruta raíz esperada dentro del proyecto es `.agents/skills/nexus-4ever/`.
